...
public final class IronBehaviour implements PipeBehaviour {
     ...
     * <p>
     * Real, pre-existing bug fixed here: this used to validate {@code currentDir} via {@code PipeBlock
     * .isConnectable} (the ITEM-pipe capability check) even when this instance is backing a FLUID pipe - since a
     * neighbouring fluid pipe/tank exposes {@code Capabilities.Fluid.BLOCK}, not {@code Capabilities.Item.BLOCK},
     * that check would almost always fail for a real fluid-pipe neighbour, making a fluid Iron pipe's facing
     * validate against the wrong capability system entirely. {@link #isValidFacing} now dispatches on which
     * concrete block entity type is actually at {@code pipePos} instead of always using the item check. */
    @Override
    public Set<Direction> filterFluidDestinations(Level level, BlockPos pipePos, Direction from, FluidResource fluid,
        ...
        if (currentDir == null || !isValidFacing(level, pipePos, currentDir)) {
            advanceFacing(level, pipePos);
        }
        return currentDir == null ? Set.of() : Set.of(currentDir);
    }
    ...
    public boolean onWrenchClick(Level level, BlockPos pipePos, @Nullable Direction clickedFace) {
        if (clickedFace != null && clickedFace != currentDir && isValidFacing(level, pipePos, clickedFace)) {
    ...
    private boolean advanceFacing(Level level, BlockPos pipePos) {
        ...
        for (int i = 1; i <= all.length; i++) {
            ...
            if (isValidFacing(level, pipePos, candidate)) {
    ...
    /** Dispatches to the item or fluid pipe connectivity check depending on which concrete block entity is
     * actually placed at {@code pipePos} - a single {@code IronBehaviour} instance only ever backs one flavour
     * for its whole lifetime (see class javadoc), so checking the real placed type is always correct. */
    private static boolean isValidFacing(Level level, BlockPos pipePos, Direction dir) {
        if (level.getBlockEntity(pipePos) instanceof FluidPipeBlockEntity) {
            return FluidPipeBlock.isConnectable(level, pipePos, dir);
        }
        return PipeBlock.isConnectable(level, pipePos, dir);
    }
